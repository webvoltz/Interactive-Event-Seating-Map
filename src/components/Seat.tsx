...
const Seats = memo(function Seats({ rows }: SeatsProps) {
    ...
    const handleInteraction = (e: React.MouseEvent | React.KeyboardEvent, seat: ISeat, isUnavailable: boolean) => {
        e.stopPropagation();
        if (!isUnavailable) {
            toggleSeat(seat.id);
    ...
    const handleKeyDown = (e: React.KeyboardEvent, seat: ISeat, rowIndex: number, seatIndex: number, isUnavailable: boolean) => {
        ...
        if (["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(e.key)) {
            ...
            if (nextRowIdx >= 0 && nextRowIdx < rows.length) {
                ...
                if (e.key === "ArrowUp" || e.key === "ArrowDown") {
                    ...
                    if (nextSeatIdx >= 0 && nextSeatIdx < currentRow.seats.length) {
                    ...
                    return (
                        ...
                        >
                            <rect
                                ...
                                stroke={stroke}
                                strokeWidth="0.3"
                                className={!isUnavailable ? "group-focus:stroke-blue-600 group-focus:stroke-[1px]" : ""}
                                style={{
